...
def main():
    ...
    weights_path = get_weights_path(args=args)

    run(device=device, weights_path=weights_path)
...
def run(device: torch.device, weights_path: str):
    ...
    for images in dataloader:
        ...
        for index in range(x.shape[0]):
            draw(x=x[index], y=y[index], y_hat=y_hat[index], index=index)


def draw(x: torch.Tensor, y: torch.Tensor, y_hat: torch.Tensor, index: int):
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(y.permute(1, 2, 0).detach().numpy(), cmap="gray")
    plt.title("Ground truth image")

    plt.subplot(1, 3, 3)
    plt.imshow(y_hat.permute(1, 2, 0).detach().numpy(), cmap="gray")
    plt.title("Generated image")

    plt.savefig(f"{current_path}/../data/outputs/output_{index}.png")
    plt.show()
