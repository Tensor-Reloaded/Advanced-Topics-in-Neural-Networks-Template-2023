...
class MetricsMemory:
    ...
    def draw_plot(self, epochs_til_now):
        x_axis = list(range(1, epochs_til_now + 1))
        # plt.plot(x_axis, self.metrics[:, 0])
        # plt.plot(x_axis, self.t_metrics[:, 0])
        plt.plot(x_axis, self.metrics[:epochs_til_now, 1])
        plt.plot(x_axis, self.t_metrics[:epochs_til_now, 1])
