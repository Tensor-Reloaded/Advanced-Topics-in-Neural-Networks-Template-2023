...
class TrainTune:
    def __init__(self, cmodel: Model,
        ...
        self.writer = writer
        self.config = config
        self.vgg16 = torchvision.models.vgg16(weights=VGG16_Weights.IMAGENET1K_V1)
    ...
    def train(self, epoch: int):
        ...
        for features, labels in self.train_loader:
            ...
            outputs = torch.nn.Upsample(scale_factor=(2, 2))(outputs)
            outputs = torch.nn.Upsample(scale_factor=(2, 2))(outputs)
            outputs = torch.nn.Upsample(scale_factor=(2, 2))(outputs)
            labels = torch.nn.Upsample(scale_factor=(2, 2))(labels)
            labels = torch.nn.Upsample(scale_factor=(2, 2))(labels)
            labels = torch.nn.Upsample(scale_factor=(2, 2))(labels)
            labels = torch.cat((labels, labels, labels), dim=1)
            outputs = torch.cat((outputs, outputs, outputs), dim=1)
            outputs = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                           std=[0.229, 0.224, 0.225])(outputs)
            labels = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                          std=[0.229, 0.224, 0.225])(labels)
            for index, layer in enumerate(self.vgg16.features):
                outputs = layer(outputs)
                labels = layer(labels)
                if index in [2, 7, 14, 21, 28]:
                    loss += (((outputs - labels) ** 2).sum() /
                             (len(outputs[0, 0]) * len(outputs[0, 0, 0]) * layer.out_channels))
