...
def main(device=get_default_device()):
    ...
    # SGD with SAM
    #optimizer = SAM(model.parameters(), base_optimizer=optim.SGD, lr=0.1, momentum=0.9)
    ...
    writer.add_scalar("Learning rate Adam", 0.1)
    ...
    for epoch in tbar:
        acc, train_loss, loss_per_batch, acc_val, val_loss = do_epoch(model, train_loader, val_loader, criterion,
                                                                      optimizer, device)
        tbar.set_postfix_str(f"Acc: {acc}, Train loss: {train_loss}, Acc_val: {acc_val}, Validation loss: {val_loss}")

        # log metrics to wandb
        wandb.log({"epoch": epoch, "train_loss": train_loss, "train_accuracy": acc,
                   "validation_loss": val_loss, "validation_accuracy": acc_val})
        ...
        writer.add_scalar("epoch_validation_accuracy", acc_val, epoch)
