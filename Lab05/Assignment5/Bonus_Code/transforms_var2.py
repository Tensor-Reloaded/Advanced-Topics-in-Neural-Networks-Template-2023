        ...
        if type(sample) == list:
            ...
            for instance in self.instances:
                for channel in range(3):
                    std, mean = torch.std_mean(sample[instance][channel])
                    sample[instance][channel] -= mean
                    sample[instance][channel] /= std
        else:
            for channel in range(3):
                std, mean = torch.std_mean(sample[channel])
                sample[channel] -= mean
                sample[channel] /= std
        return sample
