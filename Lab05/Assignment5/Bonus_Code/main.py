...
if __name__ == "__main__":
    ...
    # model.activations = [torch.nn.ReLU(), -1, -1, torch.nn.ReLU(), -1, torch.nn.Softmax()]
    # model.lr_scheduler = None
    # model.dropouts = torch.nn.ModuleList([torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       torch.nn.Dropout(0.1).to(utils.get_default_device()),
    #                                       ])
    # model.batch_norms = torch.nn.ModuleList([torch.nn.BatchNorm1d(784).to(utils.get_default_device()),
    #                                          torch.nn.BatchNorm1d(4000).to(utils.get_default_device()),
    #                                          torch.nn.BatchNorm1d(1000).to(utils.get_default_device()),
    #                                          torch.nn.BatchNorm1d(784).to(utils.get_default_device()),
    #                                          torch.nn.BatchNorm1d(4000).to(utils.get_default_device()),
    #                                          torch.nn.BatchNorm1d(10).to(utils.get_default_device()),
    #                                          ])
    # model.layers.append(torch.nn.Linear(10, 10).to(utils.get_default_device()))
    # torch.nn.init.xavier_normal_(model.layers[-1].weight)
    ...
    runner.run_model(model, transforms=[v2.ToImage(),
                     ...
                     transforms_not_cached=[
                         ...
                         RandomAugmentation(instances=[0], num_ops=3, magnitude=14),
                         ToGrayscale(instances=[0]),
                         Flatten(instances=[0]),
                     ],
