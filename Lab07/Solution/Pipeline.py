...
def sweep_id(sweep_config):
    sweep_id = wandb.sweep(sweep_config, project="Homework6_Solution")
    return sweep_id
...
class CNN(torch.nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.conv1 = torch.nn.Conv2d(input_size, 32, kernel_size=3, padding=1)
        ...
        # init.kaiming_normal_(self.conv5.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x):
        x1 = self.pool1(self.bn1(self.relu(self.conv1(x))))
        x2 = self.pool2(self.bn2(self.relu(self.conv2(x1))))
        x3 = self.bn3(self.relu(self.conv3(x2)))
        x4 = self.bn4(self.relu(self.conv4(x3)))
        x5 = self.pool5(self.bn5(self.relu(self.conv5(x4)))) # skip connections
        x6 = self.flatten(x5)
        x7 = self.dropout1(self.relu(self.fc1(x6)))
        x8 = self.dropout2(self.relu(self.fc2(x7)))
        x9 = self.fc3(x8)
        return x9
...
def build_dataset():
    ...
    train_dataset = CIFAR100(root=data_path, train=True, transform=train_transforms, download=True)
    val_dataset = CIFAR100(root=data_path, train=False, transform=transforms, download=True)
...
def build_cnn_model(device, input_size, output_size):
...
def tune(config=None):
    ...
    model = build_cnn_model(device, input_size, output_size)
