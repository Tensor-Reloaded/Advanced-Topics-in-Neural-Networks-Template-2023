...
class HandmadeConv2dImplementation(torch.nn.Module):
    def __init__(self, weights):
        ...
        self.out_channels, self.in_channels, self.kernel_size_h, self.kernel_size_w = weights.shape

    def forward(self, x):
        batch_size, _, height, width = x.shape
        out_height = height - self.kernel_size_h + 1
        out_width = width - self.kernel_size_w + 1

        output = torch.zeros(batch_size, self.out_channels, out_height, out_width)
        for i in range(out_height):
            for j in range(out_width):
                patch = x[:, :, i:i + self.kernel_size_h, j:j + self.kernel_size_w]
                output[:, :, i, j] = torch.sum(patch * self.weights, dim=(1, 2, 3)) 
...
class CustomCNN(torch.nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(128 * 32 * 32, 10)  # 10 output units for CIFAR-10 classes
...
def set_config():
    ...
    sweep_id = wandb.sweep(sweep_config, project="homework6")
    return sweep_id
...
def apply_config(config=None):
    ...
    data_path = r'D:\\Facultate\\Master1\\ACNN\\Teme\\Homework6'
...
if __name__ == '__main__':
    #Conv2d by hand, using tensor operations
