...
def load_dataset(path: str = "./data", train: bool = True, pin_memory: bool = True):
    ...
    for image, label in mnist_trainset:
        ...
        if train:
            ...
            img_rotated_1 = imutils.rotate(img_shifted_1.numpy(), 5)
            ...
            img_rotated_1 = imutils.rotate(img_shifted_3.numpy(), 5)
            img_rotated_2 = imutils.rotate(img_shifted_3.numpy(), -5)
            x_data += [torch.from_numpy(img_rotated_1), torch.from_numpy(img_rotated_2)]
            y_data += [label] * 2
            img_rotated_1 = imutils.rotate(img_shifted_3.numpy(), 10)
            img_rotated_2 = imutils.rotate(img_shifted_3.numpy(), -10)
            x_data += [torch.from_numpy(img_rotated_1), torch.from_numpy(img_rotated_2)]
            y_data += [label] * 2
            img_shifted_4 = torch.cat((tensor[:, :26], 255 * torch.ones(size=(28, 2))), dim=1)
            img_rotated_1 = imutils.rotate(img_shifted_4.numpy(), 5)
