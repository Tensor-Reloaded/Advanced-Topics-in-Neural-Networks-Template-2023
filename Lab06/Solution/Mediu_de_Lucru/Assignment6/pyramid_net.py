...
class PyramidNet(Model):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=True,
                 ...
                 layers: List = None,
                 loss=nn.BCELoss(), weight_initialization=None, to_initialize=None,
                 ...
                 lr_scheduler_args: List[dict] = None
                 ):
        super(PyramidNet, self).__init__(optimizers=optimizers, optimizer_args=optimizer_args,
                                         lr_scheduler=lr_scheduler, default_optim_args=default_optim_args,
                                         closure=closure, lr_scheduler_args=lr_scheduler_args,
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.conv1 = nn.Conv2d(3, self.input_featuremap_dim,
                                   kernel_size=3, stride=1, padding=1, bias=False).to(self.device)
            self.bn1 = nn.BatchNorm2d(self.input_featuremap_dim).to(self.device)

            self.featuremap_dim = self.input_featuremap_dim
            self.layer1 = self.pyramidal_make_layer(block, n).to(self.device)
            self.layer2 = self.pyramidal_make_layer(block, n, stride=2).to(self.device)
            self.layer3 = self.pyramidal_make_layer(block, n, stride=2).to(self.device)

            self.final_featuremap_dim = self.input_featuremap_dim
            self.bn_final = nn.BatchNorm2d(self.final_featuremap_dim).to(self.device)
            self.relu_final = nn.ReLU(inplace=True).to(self.device)
            self.avgpool = nn.AvgPool2d(8).to(self.device)
            self.fc = nn.Linear(self.final_featuremap_dim, num_classes).to(self.device)
    ...
    def pyramidal_make_layer(self, block, block_depth, stride=1):
        ...
        if stride != 1:  # or self.inplanes != int(round(featuremap_dim_1st)) * block.outchannel_ratio:
            downsample = nn.AvgPool2d((2, 2), stride=(2, 2), ceil_mode=True).to(self.device)
