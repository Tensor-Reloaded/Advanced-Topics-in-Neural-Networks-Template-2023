...
class ResnetCustom(Model):
    def __init__(self, model, **kwargs):
        ...
        added_8 = torch.nn.Linear(512, 100).to(self.device)
        ...
        self.layers = torch.nn.Sequential(upsample1,
                                          upsample2,
                                          upsample3,
                                          model.conv1.to(self.device),
                                          model.bn1.to(self.device),
                                          model.relu.to(self.device),
                                          model.layer1.to(self.device),
                                          model.layer2.to(self.device),
                                          model.layer3.to(self.device),
                                          model.layer4.to(self.device),
                                          model.maxpool.to(self.device),
                                          model.avgpool.to(self.device),
                                          added_1,
                                          added_4,
                                          added_2,
                                          added_3,
