...
class TrainTune:
    ...
    def train(self, epoch: int):
        ...
        for features, labels in self.train_loader:
            batch = batch + 1
            features, labels = v2.CutMix(num_classes=self.no_class)(features, labels)
            # labels = torch.nn.functional.one_hot(labels, num_classes=self.no_class)
            ...
            outputs = self.model_forward(features).float()
    ...
    def val(self):
        ...
        with torch.no_grad():
            for features, labels in self.val_loader:
                ...
                correct += (self.similarity(outputs, torch.nn.functional.one_hot(
                    labels, num_classes=self.no_class).float()))
                total_loss += self.model.loss(outputs, torch.nn.functional.one_hot(
                    labels, num_classes=self.no_class).float()).item()
        return total_loss, correct
    ...
    def run(self, n: int):
        ...
        for epoch in range(n):
            ...
            #                        correct_train / (len(self.val_loader) * self.val_loader.batch_size), epoch)
